...
function updateFileList(files) {
    ...
    files.forEach(file => {
        if (existingFiles.has(file.filename)) {
            ...
            link.dataset.filename = encodeURIComponent(file.filename);
            link.dataset.peerId = file.peerId;
            link.dataset.mimeType = file.type;
            link.textContent = 'Download';
            
            textSpan.appendChild(link);
            textSpan.appendChild(document.createTextNode(` (Expires in ${file.expires_in} seconds)`));
